...
from modules.device_utils import resolve_device, infer_dtype, safe_module_to_device
from gradio_utils import adapt_legacy_streaming_pairs
...
def load_v2_models(args):
    ...
    vc_wrapper = safe_module_to_device(vc_wrapper, device)
    vc_wrapper.eval()
...
def convert_voice_v1_wrapper(source_audio_path, target_audio_path, diffusion_steps=10,
    ...
    yield from adapt_legacy_streaming_pairs(
        vc_wrapper_v1.convert_voice(
...
def _wire_predict_button(fn, inputs, outputs, examples):
    """Explicit Blocks wiring (avoids broken Interface.render() in Tabs)."""
    convert_btn = gr.Button("Convert / 转换", variant="primary")
    convert_btn.click(fn=fn, inputs=inputs, outputs=outputs)
    if examples:
...
def build_v1_tab():
    ...
    gr.Markdown("## Seed Voice Conversion V1 (Voice & Singing Voice Conversion)")
    ...
    _wire_predict_button(convert_voice_v1_wrapper, inputs, outputs, examples)
...
def build_v2_tab():
    ...
    gr.Markdown("## Seed Voice Conversion V2 (Voice & Style Conversion)")
    gr.Markdown(description)
    ...
    _wire_predict_button(convert_voice_v2_wrapper, inputs, outputs, examples)
