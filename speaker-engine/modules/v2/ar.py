...
class BaseTransformer(nn.Module):
    ...
    def setup_caches(
        self, max_batch_size: int, max_seq_len: int, dtype: torch.dtype = torch.bfloat16, device: torch.device = None
    ):
        if device is None:
...
class NaiveWrapper(nn.Module):
    ...
    def setup_caches(self, max_batch_size: int, max_seq_len: int, dtype: torch.dtype = torch.bfloat16, device: torch.device = None):
