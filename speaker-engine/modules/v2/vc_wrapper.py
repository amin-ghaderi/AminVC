...
from modules.device_utils import resolve_device, infer_dtype, use_torch_compile_inductor
...
class VoiceConversionWrapper(torch.nn.Module):
    ...
    def compile_ar(self):
        ...
        """
        use_inductor = use_torch_compile_inductor()
        self.compiled_decode_fn = torch.compile(
            self.ar.model.forward_generate,
            fullgraph=True,
            backend="inductor" if use_inductor else "aot_eager",
            mode="reduce-overhead" if use_inductor else None,
        )
    ...
    def convert_voice_with_streaming(
        ...
        Args:
            ...
            device: Device to use (default: resolve_device())
            dtype: Data type to use (default: infer_dtype(device))
            stream_output: Whether to stream the output (default: True)
            for_gradio: Use Gradio-safe streaming pairs (gr.skip for partial WAV slot)
        ...
        def _yield_stream_pair(mp3_bytes, full_audio):
            if for_gradio and full_audio is None:
                from gradio_utils import gradio_skip_final
                return mp3_bytes, gradio_skip_final()
